...
xlm = df[['horsepower','curb-weight','engine-size','highway-mpg']]	#linear regression variables
ylm = df['price']							#linear regression dependent variable

ylmrm = lm.fit(xlm,ylm)							#fit linear regression model
ylmhat = lm.predict(xlm)						#prediction using above model

scores = cross_val_score(ylmrm,xlm,ylm,cv=3)				#use training set and testing set in 3 folds to give R values for model as out-sample data not in-sample data

print(scores)
#print(lm.score(xlm,ylm))						#get R^2 value (result of 1 is best)
...
f8 = np.polyfit(x4, y4, 3)					#polynomial regression model
p8 = np.poly1d(f8)						#
x_poly = pr.fit_trasnform(x[['horsepower','curb-weight'])	#
"""

ylmmse = mean_squared_error(df['price'],ylmhat)

x_train, x_test, y_train, y_test = train_test_split(xlm, ylm, test_size=0.3, random_state=0)    #split data into training and testing data
r2test = []                                                     #check R^2 values for each polynomial regression to determine the best fitting model (highest R^2 value)
...
for n in order:                                                 #
        ...
        r2test.append(lm.score(x_test_pr, y_test))              #
                                                                #
print (r2test)                                                  #
